...
export class Game {
  ...
  constructor(
     ...
     * @param id : string
     */
    public activatePlayer(id: string = this.state.active_player, timestamp?: number) {
        ...
        if (this.state.players.get(id) === undefined) {
            ...
            this.state.active_start = timestamp === undefined ? new Date().getTime() : timestamp;
        }
    }
    ...
    public stopPlayer(add_bonus_time: boolean = false, timestamp?: number) {
        ...
        if (this.state.active_player !== undefined) {
            ...
            let stop_time = timestamp === undefined ? new Date().getTime() : timestamp;
            this.state.players.get(this.state.active_player!)!.time +=
                time_bonus -
                (stop_time - this.state.active_start!);
            this.has_winner(
     ...
     */
    public insert(column: number, player_id?: string, timestamp?: number): GameStep | undefined {
        if (this.state.state !== GameStateEnum.IN_PROGRESS ||
  ...
  public get getGameState(): GameState {
    ...
    public getCurrentStep(): number {
        return this.active_step === undefined ? this.state.steps.length-1 : this.active_step;
    }
